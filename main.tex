%!TEX spellcheck = de
\documentclass[german,aspectratio=169,titlestyle=tud]{tudbeamer}%note: when switching to the tud titlestyle, an additional build is needed
...
\affiliation[ET-IT $\bullet$ IfA]{Fakultät Elektrotechnik und Informationstechnik $\bullet$ Institut für Automatisierungstechnik}
...
%% Comment if no part page shall be inserted
%\AtBeginPart{\setparttemplates\frame{\partpage}\setbeamertemplates}
...
\frame{
	...
	% 		\centering\includegraphics[keepaspectratio=1,width=.85\textwidth]{pics/TOC0}
	%     \end{column}
 %    \end{columns}
...
% Switch for starting a section without the table of contents section page
\newif\ifsectiontoc
\sectiontoctrue
\newif\ifinsertsectionimage
\insertsectionimagefalse
\newcommand{\theSectionImage}{}
\newcommand{\setSectionImage}[1]{%
	\renewcommand{\theSectionImage}{#1}%
	\@insertsectionimagetrue%
}
% \let\sectionOld\section%
% \newcommand\sectionOldStar[1]{\sectionOld*{#1}}
% \renewcommand{\section}[1]{\@ifstar{\sectionOld{#1}}{\sectionOldStar{#1}}\setboolean{insertsectionimage}{false}}

%% enabling a section introduction page, remove if not required
\AtBeginSection[]{
	\ifsectiontoc%
		\frame[squeeze]{  %\frame<beamer>[squeeze]{
		% 	\sectionpage
			{\usebeamerfont{section title}\insertsection}%
				\ifinsertsectionimage%
					\vspace*{-4ex}\begin{columns}
					    \begin{column}{0.45\textwidth}
					        \tableofcontents[currentsection,sectionstyle=show/shaded,subsectionstyle=show/show/hide]%
					    \end{column}
					    \hspace*{-.15\textwidth}\begin{column}{0.55\textwidth}
					        \centering%
					        \includegraphics[keepaspectratio=true,width=0.9\textwidth]{\theSectionImage}%
					    \end{column}					    
					\end{columns}
				\else%
					\tableofcontents[currentsection,sectionstyle=show/shaded,subsectionstyle=show/show/hide]%
...
%% Use this command for starting a section without a table of contents
\newcommand{\toclesssection}[1]{
	\sectiontocfalse
	\section{#1}
	\sectiontoctrue
}
%% Use this command for starting a starred section without a table of contents
\newcommand{\toclesssectionstar}[1]{
	\sectiontocfalse
	\section*{#1}
	\sectiontoctrue
}
