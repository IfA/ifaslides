%!TEX spellcheck = de
\documentclass[german,notoc]{tudbeamer}%note: when switching to the tud titlestyle, an additional build is needed
...
% Einstellungen für Beispielfolien
	...
	\lstdefinestyle{latex}{
		...
		moredelim=*[is][\color{tudblue}]{*b}{*},
		moredelim=*[is][\color{emph2_2}]{*i}{*},
...
\begin{frame}[fragile,label=movies]
	...
	\begin{columns}[T,onlytextwidth]
		...
		\end{column}
		\begin{column}{.475\textwidth}
		\movie[label=space,width=\textwidth,externalviewer]{\includegraphics[width=1\textwidth]{pics/movie-backup}}{pics/the-space-shuttle-rocket-launch.mp4}\\
	...
	\begin{itemize}
		\item Einbinden einer *.bib-Datei:
			\begin{lstlisting}[gobble=8,style=latex]
				...
				\addbibresource{*sbibliography.bib*}
			\end{lstlisting}
		\item Alternativ: können Literaturstellen direkt im Dokument deklariert werden, siehe 
			\hyperlink{http://www.ctan.org/tex-archive/macros/latex/contrib/beamer/doc/beameruserguide.pdf}{\beamergotobutton{Offizielles Benutzerhandbuch}} der Beamer-Klasse auf Seite 26.
...
\begin{frame}[fragile]\frametitle{Zitierung und Kurzbeleg}
    ...
    \begin{columns}[T,onlytextwidth]
		...
		\begin{column}{.5\textwidth}	
			...
			\begin{itemize}
				\item \textquote{\citetitle{baronHandreichungZitieren2018}}\cite{baronHandreichungZitieren2018}
				\item \textquote{\citetitle{SLUBZitierenHandreichung2018}}\cite{SLUBZitierenHandreichung2018}
				\item \textquote{\citetitle{SchwenkeBildzitate2012}}\cite{SchwenkeBildzitate2012}
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{finalframe}
	\vskip4ex\begin{columns}
	    ...
	    \begin{column}{0.3\textwidth}
	        \flushright{\includegraphics[keepaspectratio=true,width=.6\columnwidth]{IfA_logo_weiss}}
        	\vspace{1ex}\\
...
\begin{backmatterframe}
	...
	\vskip4ex
		Bei Fragen, Problemen und Verbesserungen,\\ kann der Issue-Tracker\\ im 
		\href{%
		...
		verwendet werden. Wenn Sie keinen Zugang haben, schicken Sie mir eine E-Mail: \texttt{lukas.baron@tu-dresden.de}.
\end{backmatterframe}
...
% appendix is not yet implemented properly
% \appendix
% \begin{frame}[fragile]
% 	\frametitle{Appendix}
